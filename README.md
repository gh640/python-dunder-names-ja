...
Python では `__init__()` のように先頭と末尾がアンダースコア 2 つ（ `__` ）の名前に特別な意味を持つものがあります。
それらは「特殊 XXX 」や「ダンダー XXX 」（ dunder = double underscore ）と呼ばれたりします（ XXX には「アトリビュート」「メソッド」などが入ります）。

ここではそれらのダンダーな名前についてまとめています。

それぞれの説明はあくまでも概説なので、厳密な仕様・挙動については Python 公式のドキュメントや PEP やソースコードで確認するようにしてください。
